...
const LabCharts: React.FC<LabChartsProps> = ({ tckn }) => {
  ...
  const isWithinNormalRange = (value: string, min: string, max: string): boolean => {
    try {
        ...
        <div className="card-body">
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">No laboratory results found for {selectedTest}</p>
            ...
            <div className="overflow-x-auto">
              <table className="table">
                ...
                <tbody>
                  {labData
                    .sort((a, b) => new Date(b.TEST_TARIHI).getTime() - new Date(a.TEST_TARIHI).getTime())
                    .map((test, index) => (
                      <tr key={index}>
                        <td className="font-medium">
                          {new Date(test.TEST_TARIHI).toLocaleDateString()}
                        </td>
                        <td className={`font-semibold ${getValueColor(test.SONUC, test.NORMAL_MIN, test.NORMAL_MAX)}`}>
                        ...
                        <td>
                          {isWithinNormalRange(test.SONUC, test.NORMAL_MIN, test.NORMAL_MAX) ? (
