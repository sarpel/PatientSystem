...
class Settings(BaseSettings):
    ...
    ai_routing_strategy: str = Field(
        default="smart",
        description="AI routing strategy (smart, manual, round_robin)"
    )
    ...
    ai_max_retries: int = Field(
        default=3,
        description="Maximum retry attempts per AI provider"
    )
    ...
    ai_temperature: float = Field(
        default=0.5,
        description="AI sampling temperature (0.0-1.0)"
    )
    ...
    enable_encryption: bool = Field(
        default=False,
        description="Enable encryption (disabled for personal use)"
    )

    # Clinical Thresholds and Constants
    crp_severe_threshold: float = Field(
        default=50.0,
        description="CRP threshold for severe inflammation (mg/L)"
    )
    ...
    fever_temperature_threshold: float = Field(
        default=38.0,
        description="Temperature threshold for fever (°C)"
    )
    hypertension_systolic_threshold: int = Field(
        default=140,
        description="Systolic BP threshold for hypertension (mmHg)"
    )
    hypertension_diastolic_threshold: int = Field(
        default=90,
        description="Diastolic BP threshold for hypertension (mmHg)"
    )
    tachycardia_threshold: int = Field(
        default=100,
        description="Heart rate threshold for tachycardia (bpm)"
    )
    obesity_bmi_threshold: float = Field(
        default=30.0,
        description="BMI threshold for obesity"
    )
    overweight_bmi_threshold: float = Field(
        default=25.0,
        description="BMI threshold for overweight"
    )
    underweight_bmi_threshold: float = Field(
        default=18.5,
        description="BMI threshold for underweight"
    )

    # AI Model Limits
    ai_max_retry_attempts: int = Field(
        default=3,
        description="Maximum retry attempts for AI providers"
    )
    ai_retry_delay_multiplier: float = Field(
        default=1.0,
        description="Base multiplier for retry delay (seconds)"
    )
    ...
    api_response_timeout: int = Field(
        default=30,
        description="API response timeout (seconds)"
    )
